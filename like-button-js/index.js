...
(() => {
  ...
  function likeButton(buttonEl) {
    let liked = false;
    let pending = false;

    function likeUnlikeAction() {
      pending = true;
      update();
    ...
    function update() {
      buttonEl.classList.toggle("like-button--liked", liked);
      buttonEl.classList.toggle("like-button--default", !liked);
      buttonEl.disabled = pending;

      buttonEl.querySelector(".like-button-icon").innerHTML = pending
        ? spinnerIconHTML
        : heartIconHTML;

      buttonEl.querySelector(".like-button-label").innerHTML = liked
        ? "Liked"
        : "Like";
    }
